...
void processNegativeInput(int a, vector<pair<int, int>>& instructions, multiset<int>& vacant, priority_queue<pair<int, int>>& queue) {
    a = abs(a);
...
void processPositiveInput(int a, vector<pair<int, int>>& instructions, multiset<int>& vacant, priority_queue<pair<int, int>>& queue, int n) {
    ...
    while ((vacant.find(front.second) == vacant.end() || upper - front.second + 1 != front.first) && !queue.empty()) {
        front = queue.top();
        queue.pop();
        upper = n;
...
int main() {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        cin >> a;
        if (a <= 0) {
            processNegativeInput(a, instructions, vacant, queue);
        } else {
            processPositiveInput(a, instructions, vacant, queue, n);
        }
    }
}